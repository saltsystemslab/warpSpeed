...
- `fill_test`: Iteratively inserts, queries, deletes, and then queries the same key in a loop 10,000 times. This tests basic safety on insertions, deletions, and queries, i.e. you should always insert a key into an empty table, find a key that has been inserted, delete an inserted key, and not find a key that has been deleted. The loop ensures that tombstones are safely replacable, as otherwise most probing schemes will fail to find a valid spot for insertion.