#ifndef OUR_IHT_P2_METADATA
#define OUR_IHT_P2_METADATA
...
#define METADATA_FRONT_TOTAL_RATIO .9
...
namespace tables {
   ...
   __device__ iht_packed_tags iht_load_multi_tags(const uint16_t * start_of_bucket){


      iht_packed_tags load_type;
   ...
   struct iht_md_backyard_bucket {

      //uint64_t lock_and_size;

      using pair_type = md_iht_p2_pair<Key, Val>;
      ...
      __device__ pair_type load_packed_pair(int index){
         ...
         return ht_load_packed_pair<md_iht_p2_pair, Key, Val>(&slots[index]);
   ...
   __global__ void init_frontyard_md_iht_p2_table_kernel(table * hash_table){
   ...
   template <typename Key, Key defaultKey, Key tombstoneKey, typename Val, Val defaultVal, Val tombstoneVal, uint partition_size, uint bucket_size>
   struct iht_md_frontyard_bucket {
   ...
   struct iht_metadata_bucket {
      ...
      #endif
      {
         ...
         //uint4 tags = iht_load_multi_tags(metadata);
      ...
      __device__ void load_fill_ballots_huge(const cg::thread_block_tile<partition_size> & my_tile, const Key & upsert_key, __restrict__ uint32_t & empty_match, __restrict__ uint32_t & tombstone_match, __restrict__ uint32_t & key_match){
         ...
         for (uint i = my_tile.thread_rank(); i < (n_traversals-1)/8+1; i+=my_tile.size()){
            ...
            if (valid_to_load){
               ...
               iht_packed_tags loaded_pair = iht_load_multi_tags(&metadata[i*8]);
   ...
   template <typename Key, Key defaultKey, Key tombstoneKey, typename Val, Val defaultVal, Val tombstoneVal, uint partition_size, uint bucket_size>
   struct frontyard_md_iht_p2_table {


      using my_type = frontyard_md_iht_p2_table<Key, defaultKey, tombstoneKey, Val, defaultVal, tombstoneVal, partition_size, bucket_size>;


      using tile_type = cg::thread_block_tile<partition_size>;


      using md_bucket_type = iht_metadata_bucket<Key, defaultKey, tombstoneKey, Val, tombstoneVal, partition_size, bucket_size>;

      using frontyard_bucket_type = iht_md_frontyard_bucket<Key, defaultKey, tombstoneKey, Val, defaultVal, tombstoneVal, partition_size, bucket_size>;

      using backyard_bucket_type = iht_md_backyard_bucket<Key, defaultKey, tombstoneKey, Val, defaultVal, tombstoneVal, partition_size, bucket_size>;

      using packed_pair_type = md_iht_p2_pair<Key, Val>;
      ...
      static __host__ my_type * generate_on_device(uint64_t cache_capacity, uint64_t ext_seed){
         ...
         host_version->n_buckets_primary = ext_n_buckets*METADATA_FRONT_TOTAL_RATIO;
         host_version->n_buckets_alt = ext_n_buckets*(1.0-METADATA_FRONT_TOTAL_RATIO)+1;
         ...
         //this is the issue
         init_frontyard_md_iht_p2_table_kernel<my_type><<<(n_buckets-1)/256+1,256>>>(device_version);
      ...
      __device__ bool upsert_generic_internal(const tile_type & my_tile, const Key & key, const Val & val, uint64_t bucket_primary, uint64_t key_hash){
         ...
         //if (my_tile.thread_rank() == 0) printf("after md load\n");
      ...
      __device__ bool find_with_reference(tile_type my_tile, Key key, Val & val){
         ...
         //including unlock 3 touches for above - this is correct.


         //extra number of touches here is because each group now can only touch a cache line halfway
         //need 2 touches to perform. 
      ...
      static char * get_name(){
         return "iht_p2_metadata_hashing";
      }
      ...
      __host__ void print_space_usage(){
         ...
         uint64_t capacity = host_version->n_buckets_primary*sizeof(frontyard_bucket_type) + (host_version->n_buckets_primary-1)/8+1; 

         capacity += host_version->n_buckets_alt*sizeof(backyard_bucket_type);


         cudaFreeHost(host_version);

         printf("iht_p2_metadata_hashing using %llu bytes\n", capacity);
...
template <typename T>
constexpr T generate_md_iht_p2_tombstone(uint64_t offset) {
  return (~((T) 0)) - offset;
};

template <typename T>
constexpr T generate_md_iht_p2_sentinel() {
  return ((T) 0);
};
...
using iht_p2_metadata_generic = typename hashing_project::tables::frontyard_md_iht_p2_table<Key,
                                    generate_md_iht_p2_sentinel<Key>(),
                                    generate_md_iht_p2_tombstone<Key>(0),
                                    Val,
                                    generate_md_iht_p2_sentinel<Val>(),
                                    generate_md_iht_p2_tombstone<Val>(0),
                                    tile_size,
                                    bucket_size>;
