...
#include <hashing_project/helpers/const_cuckoo_vector.cuh>
...
namespace tables {
   ...
   struct cuckoo_table {
      ...
      using vector_type = hashing_project::data_structs::cuckoo_vector<hashing_project::data_structs::vector_pair<Key>, CUCKOO_MAX_PROBES+1>;
      ...
      __device__ bucket_type * get_bucket_ptr_alt(uint64_t bucket_addr){
       ...
       __device__ bool upsert_generic(const tile_type & my_tile, const Key & key, const Val & val){
         ...
         __shared__ vector_type sort_vectors[32];
         ...
         vector_type * sorted_hashes = &sort_vectors[my_tile.meta_group_rank()];

         if (my_tile.thread_rank() == 0){

            my_vector->reset({0,0,0});
            sorted_hashes->reset({0,0,0});
         }
         ...
         while (true){
            ...
            verify_sort(my_tile, sorted_hashes);
      ...
      __device__ bool upsert_primary_buckets(const tile_type & my_tile, const Key & key, const Val & val, uint64_t bucket_0, uint64_t bucket_1, uint64_t bucket_2){
         ...
         if (bucket_0_ptr->load_fill_ballots_upserts(my_tile, key, val, bucket_0_tombstone)) return true;

         uint bucket_1_tombstone;
         bucket_type * bucket_1_ptr = get_bucket_ptr_primary(bucket_1);

         if (bucket_1_ptr->load_fill_ballots_upserts(my_tile, key, val, bucket_1_tombstone)) return true;

         uint bucket_2_tombstone;
         bucket_type * bucket_2_ptr = get_bucket_ptr_primary(bucket_2);

         if (bucket_2_ptr->load_fill_ballots_upserts(my_tile, key, val, bucket_2_tombstone)) return true;


         uint empty_buckets = 0;

         if (bucket_0_ptr->insert_ballots(my_tile, key, val, empty_buckets, bucket_0_tombstone)) return true;

         if (bucket_1_ptr->insert_ballots(my_tile, key, val, empty_buckets, bucket_1_tombstone)) return true;

         if (bucket_2_ptr->insert_ballots(my_tile, key, val, empty_buckets, bucket_2_tombstone)) return true;
