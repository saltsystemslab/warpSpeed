...
__device__ bool bght::bcht<Key, T, Hash, KeyEqual, Scope, Allocator, B>::insert(
  ...
  //if on a later round, upsert tricks do not work.
  //bool first_round = true;

  //auto bucket_id_1 = hf1_(pair.first) % num_buckets_;
  //auto bucket_id_2 = hf2_(pair.first) % num_buckets_;
  ...
  do {
    ...
    insertion_pair = tile.shfl(insertion_pair, elected_lane);
    
    auto bucket_id_0 = hf0_(insertion_pair.first) % num_buckets_;
    auto bucket_id_1 = hf1_(insertion_pair.first) % num_buckets_;
    auto bucket_id_2 = hf2_(insertion_pair.first) % num_buckets_;

    //shuffle insertion pair so all threads observe correct pairing
    //insertion_pair = pair;

    //calculate hashes
    //auto bucket_id_0 = hf0_(insertion_pair.first) % num_buckets_;

    bucket_type cur_bucket(&d_table_[bucket_id_0 * bucket_size], tile);
    INCREMENT_PROBES_IN_TILE
    cur_bucket.load(cuda::memory_order_relaxed);


    bucket_type main_bucket(&d_table_[bucket_id * bucket_size], tile);

    main_bucket.copy_other_bucket(cur_bucket);


    //upsert
    if (cur_bucket.upsert(insertion_pair, key_equal{})) return true;
    ...
    if (load == bucket_size){
      ...
      //must check upsert
      bucket_type cur_bucket_1(&d_table_[bucket_id_1 * bucket_size], tile);
      INCREMENT_PROBES_IN_TILE
      cur_bucket_1.load(cuda::memory_order_relaxed);

      if (cur_bucket_1.upsert(insertion_pair, key_equal{})) return true;

      if (bucket_id == bucket_id_1) main_bucket.copy_other_bucket(cur_bucket_1);


       bucket_type cur_bucket_2(&d_table_[bucket_id_2 * bucket_size], tile);
       INCREMENT_PROBES_IN_TILE
       cur_bucket_2.load(cuda::memory_order_relaxed);

      if (cur_bucket_2.upsert(insertion_pair, key_equal{})) return true;

      if (bucket_id == bucket_id_2) main_bucket.copy_other_bucket(cur_bucket_2);

    }


    //all three buckets loaded and main set to accurate version.
    ...
    // bucket_type main_bucket(&d_table_[bucket_id * bucket_size], tile);
    // INCREMENT_PROBES_IN_TILE
    // main_bucket.load(cuda::memory_order_relaxed);
   

    // bucket_type cur_bucket(&d_table_[bucket_id * bucket_size], tile);
    // cur_bucket.load(cuda::memory_order_relaxed);
    
    // //upsert attempt - if it succeeds done!
    

    // INCREMENT_PROBES_IN_TILE
    load = main_bucket.compute_load(sentinel_pair);
    ...
    if (load != bucket_size) {
      ...
      if (lane_id == elected_lane) {
        cas_success = main_bucket.strong_cas_at_location(insertion_pair,
                                                        load,
                                                        sentinel_pair,
      ...
      //update - attempt upsert in alt buckets.
        // auto cur_bucket1_addr = hf1_(insertion_pair.first) % num_buckets_;

        // bucket_type cur_bucket1(&d_table_[cur_bucket1_addr* bucket_size], tile);

        // cur_bucket1.load(cuda::memory_order_relaxed);
        // if (cur_bucket1.upsert(insertion_pair, key_equal{})) return true;


        // auto cur_bucket2_addr = hf2_(insertion_pair.first) % num_buckets_;

        // bucket_type cur_bucket2(&d_table_[cur_bucket2_addr* bucket_size], tile);

        // cur_bucket2.load(cuda::memory_order_relaxed);

        // if (cur_bucket2.upsert(insertion_pair, key_equal{})) return true;

      if (lane_id == elected_lane) {
        auto random_location = rng() % bucket_size;
        auto old_pair = main_bucket.exch_at_location(
