...
template <class Key,
          ...
          int B,
          int Threshold>
__host__ iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold> * iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::generate_on_device(std::size_t capacity,
                                                          Key empty_key_sentinel,
                                                          T empty_value_sentinel,
                                                          Allocator const& allocator){
  ...
  using table_type = iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>;
  ...
  host_version->capacity_ = detail::get_valid_capacity<B>(host_version->capacity_);
  ...
  host_version->hfp_ = initialize_hf<hasher>(rng);
...
bght::iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::insert(
  ...
  int load = 0;
  bucket_type bucket(&d_table_[primary_bucket * bucket_size]);
  if (threshold_ > 0) {
    bucket.load(cuda::memory_order_relaxed, tile);
    INCREMENT_PROBES_IN_TILE
    auto key_location = bucket.find_key_location(pair.first, key_equal{}, tile);
    // check if key exists
    if (key_location != -1) {
      return {bucket.begin() + key_location, false};
    }
    load = bucket.compute_load(sentinel_pair, tile);
  }
  do {
    if (load >= threshold_) {
      ...
      int n_buckets_traversed = 0;
      while (n_buckets_traversed < 20) {
        bucket = bucket_type(&d_table_[bucket_id * bucket_size]);
        bucket.load(cuda::memory_order_relaxed, tile);
...
template <class Key,
          ...
          int Threshold>
__device__ bool
bght::iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::find_by_reference(tile_type const& tile, key_type const& key,
                                                               mapped_type &value) {
   auto bucket_id = hfp_(key) % num_buckets_;
  using bucket_type = detail::bucket<atomic_pair_type, value_type, B>;
  ...
  if (threshold_ > 0) {
    ...
    if (key_location != -1) {
      value = cur_bucket.get_value_from_lane(key_location, tile);
      return true;
    }
  }
  ...
  value_type sentinel_pair{sentinel_key_, sentinel_value_};
  do {
    cur_bucket = bucket_type(&d_table_[bucket_id * bucket_size]);
    cur_bucket.load(cuda::memory_order_relaxed, tile);
    INCREMENT_PROBES_IN_TILE
    int key_location = cur_bucket.find_key_location(key, key_equal{}, tile);
    if (key_location != -1) {
      value = cur_bucket.get_value_from_lane(key_location, tile);
      return true;
    } else if (cur_bucket.compute_load(sentinel_pair, tile) < bucket_size) {
      return false;
    }
    bucket_id = (bucket_id + step_size) % num_buckets_;
    if (bucket_id == initial_bucket)
...
template <class Key,
          ...
          int B,
          int Threshold>
__device__ bool bght::iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::upsert_exact(
    tile_type const& tile,
    key_type insert_key,
  ...
  while (n_buckets_traversed < 20) {
    ...
    //early exit - would have been placed here.
    if (bucket.compute_load(sentinel_pair, tile) < B){ return false; }
...
template <class Key,
          ...
          int B,
          int Threshold>
__device__ bght::iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::value_type bght::iht<Key, T, Hash, KeyEqual, Scope, Allocator, B, Threshold>::find_smaller_hash(
    tile_type const& tile,
    Key const& key
    ) {
  ...
  const int elected_lane = 0;
  auto lane_id = tile.thread_rank();
  auto primary_bucket = hfp_(key) % num_buckets_;
  ...
  auto location = bucket.find_replace_location(key, primary_bucket, tile);
  

  if (location != -1) return bucket.get_lane_pair(location, tile);
  //main load determines shortcutting
  //tombstone load determines insert positions/fill.
  ...
  while (n_buckets_traversed < 20) {
    ...
    location = bucket.find_replace_location(key, bucket_id, tile);
    //early exit - would have been placed here.
    if (location != -1){
  ...
  //fall back on primary
  bucket = bucket_type(&d_table_[primary_bucket * bucket_size]);
  INCREMENT_PROBES_IN_TILE
  bucket.load(cuda::memory_order_relaxed, tile);

  return bucket.get_lane_pair(B-1, tile);
