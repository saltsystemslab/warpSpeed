...
struct iht {
  ...
  static __host__ iht * generate_on_device(std::size_t capacity,
...
constexpr int divide_by_three_fourths(int value) {
  return value*3/4;
};
...
template <typename Key, typename T, uint B, int Threshold = divide_by_three_fourths(B)>
using iht_generic = typename bght::iht<Key,
                                 T,
                                 bght::MurmurHash3_32<Key>,
                                 bght::equal_to<Key>,
                                 cuda::thread_scope_device,
