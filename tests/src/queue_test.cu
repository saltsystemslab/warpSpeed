...
template <typename queue_type, uint tile_size>
__global__ void queue_read_kernel(queue_type * queue, uint64_t n_indices, uint64_t * access_pattern, uint64_t n_ops){
   ...
   uint64_t my_key = access_pattern[tid] % n_indices;
   ...
   uint64_t queue_loc = queue->stall_lock_enqueue_replace(my_key, replace_key);


   queue->unlock(queue_loc);
...
__host__ void cache_test(uint64_t host_items, uint64_t n_ops, uint64_t * data_pattern){
   ...
   std::string filename = "results/queue/";

   filename += ht_type::get_name();
   ...
   for (int i = 2; i <= 20; i++){
      ...
      queue_type * cache = queue_type::generate_on_device(capacity);

      cudaDeviceSynchronize();

      gallatin::utils::timer cache_timing;

      queue_read_kernel<queue_type, tile_size><<<(n_ops*tile_size -1)/256+1,256>>>(cache, host_items, dev_data, n_ops);
      ...
      queue_type::free_on_device(cache);
...
int main(int argc, char** argv) {
   ...
   if(fs::create_directory("results/queue")){
    std::cout << "Created a directory\n";
   } else {
    std::cerr << "Failed to create a directory\n";
   }
