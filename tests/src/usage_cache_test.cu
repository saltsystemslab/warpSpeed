...
__host__ void cache_test(uint64_t host_items, uint64_t n_ops, uint64_t * data_pattern){
   ...
   std::string filename = "results/cache_counts/";
   ...
   myfile << "fill query negative insert remove empty n_ops\n";
   ...
   for (int i = 10; i <= 90; i+=10){
      ...
      myfile << .01*i << " " << helpers::get_num_cache_queries() << " " << helpers::get_num_cache_queries_negative() << " " << helpers::get_num_cache_inserts() << " " << helpers::get_num_cache_removes() << " " << helpers::get_num_cache_inserts_empty() << " " << n_ops << "\n";
...
int main(int argc, char** argv) {
   ...
   if(fs::create_directory("results/cache_counts")){
    std::cout << "Created a directory\n";
   } else {
    std::cerr << "Failed to create a directory\n";
   }
